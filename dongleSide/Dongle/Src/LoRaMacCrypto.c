 ...
 *
 */
static aes_context DongleAesContext;
...
void DonglePayloadEncrypt( const uint8_t *buffer, uint16_t size, const uint8_t *key, uint32_t address, uint8_t dir, uint32_t sequenceCounter, uint8_t *encBuffer )
{
    ...
    memset1( DongleAesContext.ksch, '\0', 240 );
    aes_set_key( key, 16, &DongleAesContext );
    ...
    while( size >= 16 )
    {
        aBlock[15] = ( ( ctr ) & 0xFF );
        ctr++;
        aes_encrypt( aBlock, sBlock, &DongleAesContext );
        for( i = 0; i < 16; i++ )
        {
            encBuffer[bufferIndex + i] = buffer[bufferIndex + i] ^ sBlock[i];
        }
...
{
    DonglePayloadEncrypt( buffer, size, key, address, dir, sequenceCounter, decBuffer );
}
